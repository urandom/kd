...
func (rt *runtime) RegisterControllerOperator(typeName k8s.ControllerType, op ControllerOperator) {
	...
	if op.List != nil {
		list := op.List
		k8sOp.List = func(c *k8s.Client, ns string, opts meta.ListOptions) (k8s.ControllerGenerator, error) {
	...
	if op.Watch != nil {
		w := op.Watch
		k8sOp.Watch = func(c *k8s.Client, ns string, opts meta.ListOptions) (watch.Interface, error) {
			type payload struct {
	...
	if op.Update != nil {
		update := op.Update
		k8sOp.Update = func(c *k8s.Client, o k8s.ObjectMetaGetter) error {
			errC := make(chan error)
	...
	if op.Delete != nil {
		delete := op.Delete
		k8sOp.Delete = func(c *k8s.Client, o k8s.ObjectMetaGetter, opts meta.DeleteOptions) error {
			errC := make(chan error)
